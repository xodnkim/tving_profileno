...
- **빠른 검증 및 배치 모드 (`quick.py`)**: 마이페이지 이동 없이 로그인 직후 API 응답을 즉시 가로채어 소요 시간을 약 10초로 단축합니다. 특히 **`accounts.json` 파일에 등록된 다수의 계정을 순차적으로 로그인하여 실제 발급된 `profileNo`가 DB 기준 기대값과 일치하는지 일괄 비교(PASS/FAIL)하는 배치 검증 기능(`--file`)**을 지원하여 실무 QA 테스트 환경에서도 바로 활용할 수 있도록 설계했습니다.

### 2) 안정적인 셀렉터 (`data-testid`) 활용
...
### 4) API 응답(Network Interception)에서 데이터 추출
